...
public class Rook : Piece
{
    ...
    public override List<Coordinate> GetPossibleMoves(GameController controller)
    {
        ...
        // implement the rock logic
        for (var i = 1; i < 8; i++)
        {
            if (!top && IsWithinBounds(Position.X + i, Position.Y))
            {
                if (controller.Board.GetPieceAt(new Coordinate(Position.X + i, Position.Y)) != null) top = true;
                possibleMoves.Add(new Coordinate(Position.X + i, Position.Y));
            }

            if (!bottom && IsWithinBounds(Position.X - i, Position.Y))
            {
                if (controller.Board.GetPieceAt(new Coordinate(Position.X - i, Position.Y)) != null) bottom = true;
                possibleMoves.Add(new Coordinate(Position.X - i, Position.Y));
            }

            if (!left && IsWithinBounds(Position.X, Position.Y + i))
            {
                if (controller.Board.GetPieceAt(new Coordinate(Position.X, Position.Y + i)) != null) left = true;
                possibleMoves.Add(new Coordinate(Position.X, Position.Y + i));
            }

            if (!right && IsWithinBounds(Position.X, Position.Y - i))
            {
                if (controller.Board.GetPieceAt(new Coordinate(Position.X, Position.Y - i)) != null) right = true;
                possibleMoves.Add(new Coordinate(Position.X, Position.Y - i));
            }
        }
